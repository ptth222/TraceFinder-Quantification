...
gen_uM_sequence_ratio_column <- function(galaxy_data, std_compound_refdb, sequence_data){
  ...
  for(i in 1:(length(std_id)-1)){
    ...
    if(std_1$position+1 != std_2$position){
      ...
      ## Filter down to only C_isommer = 0 data
...
gen_umol_g_protein_columns <- function(galaxy_data, std_compound_refdb, sequence_data, meta_data, stdmix_id, sample_id, Labelling, swap_compound = TRUE){
  ...
  galaxy_data <- gen_uM_sequence_ratio_column(galaxy_data, std_compound_refdb, sequence_data)
